...
describe("Market", function () {
  describe("split.erc20.proxy", function () {
    it("split revert", async function () {
      ...
      // split not tokenid
      await expect(info.appOperator.connect(user1).split(10, 100, max)).revertedWith("ERC721: invalid token ID");

      // split deleted tokenid
      expect(await info.mortgageNFTKol.ownerOf(1)).eq(user1.address);
      await info.appOperator
        .connect(user1)
        .redeem(1, getTokenAmountWei(10000), max);
      await expect(info.mortgageNFTKol.ownerOf(1)).revertedWith("ERC721: invalid token ID");

      await expect(info.appOperator.connect(user1).split(1, 1, max)).revertedWith("ERC721: invalid token ID");

      // split other user tokenid
      await expect(info.appOperator.connect(user1).split(5, 1, max)).revertedWith("AOE");

      // split amount not enough
      await expect(info.appOperator.connect(user1).split(2, getTokenAmountWei(30000), max)).revertedWith("SAE");

      // split amount 0
      await expect(info.appOperator.connect(user1).split(2, 0, max)).revertedWith("SAE");

      // split amount eq
      await expect(info.appOperator.connect(user1).split(2, getTokenAmountWei(20000), max)).revertedWith("SAE");
    });
    ...
    it("split self", async function () {
      ...
      let result = await info.appOperator
        .connect(user1)
        .split.staticCall(4, getTokenAmountWei(15000), max);
      await info.appOperator.connect(user1).split(4, getTokenAmountWei(15000), max);
    ...
    it("split self refund and result", async function () {
      ...
      await info.appOperator
        .connect(user1)
        .split(4, getTokenAmountWei(15000), max);
