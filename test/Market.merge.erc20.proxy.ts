...
describe("Market", function () {
  describe("merge.erc20.proxy", function () {
    it("merge revert", async function () {
      ...
      await info.simpleToken.connect(user1).approve(await info.appOperator.getAddress(), approveValue)
      await info.simpleToken.connect(user2).approve(await info.appOperator.getAddress(), approveValue)

      // user1 buy and mortgage 10000 t1 token=1
      await info.appOperator
        .connect(user1)
        .buy(paramsT1.tid, getTokenAmountWei(10000), max);
      ...
      await info.appOperator
        .connect(user1)
        .buy(paramsT1.tid, getTokenAmountWei(20000), max);
      ...
      await info.appOperator
        .connect(user1)
        .buy(paramsT2.tid, getTokenAmountWei(30000), max);
      ...
      await info.appOperator
        .connect(user1)
        .buy(paramsT2.tid, getTokenAmountWei(40000), max);
      ...
      await info.appOperator
        .connect(user2)
        .buy(paramsT1.tid, getTokenAmountWei(15000), max);
      ...
      await info.appOperator
        .connect(user2)
        .buy(paramsT1.tid, getTokenAmountWei(25000), max);
      ...
      await info.appOperator
        .connect(user2)
        .buy(paramsT2.tid, getTokenAmountWei(35000), max);
      ...
      await info.appOperator
        .connect(user2)
        .buy(paramsT2.tid, getTokenAmountWei(45000), max);
      await info.appOperator.connect(user2).mortgage(paramsT2.tid, getTokenAmountWei(45000));
      //
      // merge not tokenid
      await expect(info.appOperator.connect(user1).merge(1, 10)).rejectedWith("ERC721: invalid token ID");
      await expect(info.appOperator.connect(user1).merge(10, 1)).rejectedWith("ERC721: invalid token ID");
      await expect(info.appOperator.connect(user1).merge(9, 10)).rejectedWith("ERC721: invalid token ID");
      // merge deleted tokenid
      // redeem 7 and 3
      await info.appOperator.connect(user2).redeem(7, getTokenAmountWei(35000), max);
      await info.appOperator.connect(user1).redeem(3, getTokenAmountWei(30000), max);
      ...
      await expect(info.appOperator.connect(user1).merge(1, 7)).rejectedWith("ERC721: invalid token ID");
      await expect(info.appOperator.connect(user1).merge(7, 1)).rejectedWith("ERC721: invalid token ID");
      await expect(info.appOperator.connect(user1).merge(7, 3)).rejectedWith("ERC721: invalid token ID");

      // merge other user tokenid
      await expect(info.appOperator.connect(user1).merge(1, 5)).rejectedWith("AOE2");
      await expect(info.appOperator.connect(user1).merge(5, 1)).rejectedWith("AOE1");

      // merge other tid tokenid
      await expect(info.appOperator.connect(user1).merge(1, 4)).rejectedWith("TE");
    });
    ...
    it("merge self nft smail merge big and result", async function () {
      ...
      let user_get_payToken_1 = await info.appOperator.connect(user1).merge.staticCall(1, 2);
      await info.appOperator.connect(user1).merge(1, 2);
    ...
    it("merge self nft big merge smail", async function () {
      ...
      await info.appOperator.connect(user1).merge(2, 1);
