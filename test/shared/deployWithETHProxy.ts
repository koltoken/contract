...
import { getAllContractAddressProxy, ZERO_ADDRESS } from "./deploy_utils";

export type AllContractInfoWithETHProxy = {
  wallets: any;
...
export async function deployAllContractWithETHProxy(): Promise<AllContractInfoWithETHProxy> {
  let wallets;
  ...
  // create kol app
  await foundry.createApp("proxy", kolOwnerWallet.address, addressInfo.appOperator, await kolCurve.getAddress(), ZERO_ADDRESS, buySellFee);
