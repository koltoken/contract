...
describe("Market", function () {
  describe("multiply.erc20", function () {
    let data = [
      {
        amount: "83333.333",
        price: "0.0011901",
    ...
    it("multiply revert", async function () {
      ...
      await info.simpleToken.approve(await info.marketKol.getAddress(), BigInt(10) ** BigInt(18) * BigInt(100000000))
      ...
      // approve < need
      await expect(
        info.marketKol.multiply(paramsT1.tid, getTokenAmountWei(1000)),
    ...
    it("multiplyAdd revert", async function () {
      ...
      await info.simpleToken.approve(await info.marketKol.getAddress(), BigInt(10) ** BigInt(18) * BigInt(100000000))
      await info.simpleToken.connect(user1).approve(await info.marketKol.getAddress(), BigInt(10) ** BigInt(18) * BigInt(100000000))
      await info.simpleToken.connect(user2).approve(await info.marketKol.getAddress(), BigInt(10) ** BigInt(18) * BigInt(100000000))
      ...
      //  not tokenid
      await expect(
        info.marketKol.multiplyAdd(3, getTokenAmountWei(1000)),
      ...
      // 0
      await expect(
        info.marketKol.connect(user1).multiplyAdd(2, 0),
      ).revertedWith("TAE");
      ...
      // approve < need
      await info.simpleToken.connect(user1).approve(await info.marketKol.getAddress(), 1)
      await expect(
        info.marketKol.connect(user1).multiplyAdd(2, getTokenAmountWei(1000)),
      ).revertedWith("ERC20: insufficient allowance");
    });
    ...
    it("multiplyAdd result", async function () {
      ...
      await info.marketKol.connect(user1).multiplyAdd(1, getTokenAmountWei(1000));
    ...
    it("multiply", async function () {
      for (let i = 0; i < data.length; i++) {
        ...
        await info.marketKol.connect(user1).multiply(paramsT1.tid, amount);
    ...
    it("multiply + multiplyAdd + multiplyAdd", async function () {
      for (let i = 0; i < data.length; i++) {
        ...
        await info.marketKol.connect(user1).multiply(paramsT1.tid, part1);
        ...
        await info.marketKol.connect(user1).multiplyAdd(1, part2);
        ...
        await info.marketKol.connect(user1).multiplyAdd(1, part3);
