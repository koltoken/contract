...
describe("Market", function () {
  describe("buyAndSell.erc20.proxy", function () {
    let data = [
      {
    ...
    it("buy revert", async function () {
      ...
      await expect(
        info.appOperator.buy(params.tid, getTokenAmountWei(1000), BigInt(10) ** BigInt(18) * BigInt("10000000000")),
      ).revertedWith("ERC20: insufficient allowance");
      // buy amount > 1000W
      await info.simpleToken.approve(await info.appOperator.getAddress(), BigInt(10) ** BigInt(18) * BigInt("10000000000"))
      await expect(
        info.appOperator.buy(params.tid, getTokenAmountWei(10000000), BigInt(10) ** BigInt(18) * BigInt("10000000000"))
      ).revertedWithPanic("0x11");
      // buy 0
      await expect(info.appOperator.buy(params.tid, 0, BigInt(10) ** BigInt(18) * BigInt("10000000000"))).revertedWith("TAE");
      // not tid
      await expect(
        info.appOperator.buy("t2222", getTokenAmountWei(1000), BigInt(10) ** BigInt(18) * BigInt("10000000000")),
      ).revertedWith("TE");
    });
    ...
    it("sell revert", async function () {
      ...
      await info.simpleToken.connect(user1).approve(await info.appOperator.getAddress(), BigInt(10) ** BigInt(23))
      await info.simpleToken.connect(user2).approve(await info.appOperator.getAddress(), BigInt(10) ** BigInt(23))
      await info.appOperator
        .connect(user1)
        .buy(paramsT1.tid, getTokenAmountWei(1000), await info.simpleToken.balanceOf(user1.address));
      // user2 buy t1 and t2 1000
      await info.appOperator
        .connect(user2)
        .buy(paramsT1.tid, getTokenAmountWei(1000), await info.simpleToken.balanceOf(user2.address));
      await info.appOperator
        .connect(user2)
        .buy(paramsT2.tid, getTokenAmountWei(1000), await info.simpleToken.balanceOf(user2.address));
    ...
    it("buy refund result and sell result", async function () {
      ...
      await info.simpleToken.connect(user1).approve(await info.appOperator.getAddress(), BigInt(10) ** BigInt(18) * BigInt("10000000000"))
      let needPayToken = await info.appOperator.connect(user1).buy.staticCall(params.tid, buyAmount, BigInt(10) ** BigInt(18) * BigInt("10000000000"));

      let user1PayToken1 = await info.simpleToken.balanceOf(user1.address)

      await info.appOperator.connect(user1).buy(params.tid, buyAmount, needPayToken * BigInt(100));
      expect(await info.simpleToken.balanceOf(await info.appOperator.getAddress())).eq(0)

      let user1PayToken2 = await info.simpleToken.balanceOf(user1.address)

      expect(user1PayToken2).eq(user1PayToken1 - needPayToken);

      let userGetPayToken = await info.appOperator.connect(user1).sell.staticCall(params.tid, buyAmount);
      await info.appOperator.connect(user1).sell(params.tid, buyAmount);
    ...
    it("buy one", async function () {
      for (let i = 0; i < data.length; i++) {
        ...
        await info.appOperator.connect(user1).buy(params.tid, buyAmount, needPayToken);
    ...
    it("buy one and sell one", async function () {
      for (let i = 0; i < data.length; i++) {
        ...
        expect(await info.marketKol.balanceOf(params.tid, await info.marketKol.getAddress())).eq(0);
        expect(await info.marketKol.balanceOf(params.tid, await info.appOperator.getAddress())).eq(0);
        expect(await info.marketKol.balanceOf(params.tid, nftOwner1.address)).eq(0);
        expect(await info.marketKol.balanceOf(params.tid, nftOwner2.address)).eq(0);
